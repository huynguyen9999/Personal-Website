...
- Who I am includes a MapLibre globe of the real Earth, pinned to Santa Barbara, CA 93101. Visitor distance is estimated from IP on the server (Vercel geo headers, then IP lookup), then converted to road miles through OpenStreetMap / OSRM when a route is available. The raw IP is never shown.
